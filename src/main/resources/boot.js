var __scboot = null;
(function () {
    var File = java.io.File, FileReader = java.io.FileReader, FileOutputStream = java.io.FileOutputStream, ZipInputStream = java.util.zip.ZipInputStream, jsPlugins = new File("pvpin"),
    ...
    __scboot = function (plugin, engine, classLoader) {
        var logger = plugin.canary ? plugin.logman : plugin.logger, initScriptFile = new File(jsPlugins, initScript), zips = ["pvpin_js_rt", "pvpin-player"], i = 0, zis, len = zips.length;
        if (!jsPlugins.exists()) {
